        ...
        if (observerType == MSG_SMS_WHAT) {
            ...
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    //最后收到的短信在第一条. This method will return false if the cursor is empty
            ...
            Cursor cursor = mContext.getContentResolver().query(uri, null, "read = 0", null, "date desc");
            //Passing null will return all columns, which is inefficient.
            // 等价于附加条件
            if (cursor.getInt(cursor.getColumnIndex("read")) == 0)
                // 表示短信未读。这种方式不靠谱啊，建议用上面的方式！
                if (cursor != null) {
