...
public class CallLogObserver extends ContentObserver {
    ...
    public void onChange(boolean selfChange) {
        Cursor cursor;
        switch (type) {
            case MSG_CALLLOG_DELETE_WHAT:
                //删除最近的一条通话记录
                resolver.unregisterContentObserver(this);
                //注意：增删改通话记录后由于数据库发生变化，所以系统会在修改后再发一条广播，这时会重新回调onChange方法
                ...
                //按_id倒序排序后取第一个，即：查询结果按_id从大到小排序，然后取最上面一个（最近的通话记录）
                if (cursor != null) {
                ...
                if (cursor != null) {
                    if (cursor.moveToFirst()) {
                        ...
                        //通话类型，1 来电 .INCOMING_TYPE；2 已拨 .OUTGOING_；3 未接 .MISSED_
                         ...
                         //通话时间，即可以用getString接收，也可以用getLong接收
                         String formatDate = new SimpleDateFormat("yyyy.MM.dd HH:mm:ss", Locale.getDefault()).format(new Date(date));                    int duration = cursor.getInt(cursor.getColumnIndex("duration"));//通话时长，单位：秒                    String msgObj = "\nID：" + _id + "\n类型：" + type + "\n号码：" + number + "\n名称：" + name + "\n时间：" + formatDate + "\n时长：" + duration;                    mHandler.sendMessage(Message.obtain(mHandler, MSG_CALLLOG_QUERY_WHAT, msgObj));                }                cursor.close();            }            break;        }    }}
